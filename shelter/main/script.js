...
const petsCardContainer = document.querySelector('.slider__line');
const infoBtnPets = document.querySelectorAll('.card__btn-info');

petsCardContainer.addEventListener('click', (event) => {
    const container = event.target.closest('.card-pet');
...
function printSelfAssessmentToConsole() {
    console.log("Ваша оценка - 97.5 баллов\n",
    "блок 'Help': Частично выполненные пункты: Отзыв: не соответствует разметке перфект пиксель\n")
}

printSelfAssessmentToConsole()