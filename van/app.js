const keyData = [
  [
    ...
    { key: { ru: 'Backspace' }, code: 'Backspace', class: 'key-backspace', noType: true}
  ],

  [
    { key: { ru: 'Tab' }, code: 'Tab', class: 'key-tab', noType: true},
  ...
  [
    { key: { ru: '<span class="caps_icon">•</span><div>Caps<br>lock</div>' }, code: 'CapsLock', class: 'key-capslock', noType: true},
    ...
    { key: { ru: 'Enter' }, code: 'Enter', class: 'key-enter', noType: true}
  ],

  [
    { key: { ru: 'Shift' }, code: 'ShiftLeft', class: 'key-leftshift', noType: true},
    ...
    { key: { ru:'↑' }, code: 'ArrowUp', noType: true }, 
    {key: {ru: 'Shift'}, code: 'ShiftRight', class: 'key-rightshift', noType: true}
  ],

  [
    { key: { ru: 'Ctrl' }, code: 'ControlLeft', class: 'key-leftctrl', noType: true},
    { key: { ru: 'RU', en: 'EN' }, code: 'Lang', class: 'key-lang', noType: true},
    { key: { ru: 'Alt' }, code: 'AltLeft', class: 'key-leftalt', noType: true},
    { key: { ru: ' ' }, code: 'Space', class: 'key-space' }, 
    { key: { ru: 'Alt' }, code: 'AltRight', class: 'key-rightalt', noType: true}, 
    { key: { ru: 'Ctrl' }, code: 'ControlRight', class: 'key-rightctrl', noType: true},
    { key: { ru:  '←' }, code: 'ArrowLeft', noType: true },
    { key: { ru:  '↓' }, code: 'ArrowDown', noType: true }, 
    { key: { ru: '→' }, code: 'ArrowRight', noType: true }
  ]
];

const body = document.querySelector('body');
const container = createDomElem('div', '', 'container');
const textarea = createDomElem('textarea', '', 'textarea');
const keyboard = createDomElem('div', '', 'keyboard');
...
function createDomElem(element, innerHTML, styles) {

  const domEl = document.createElement(element);
  domEl.innerHTML = innerHTML;
  domEl.classList.add(styles);
  return domEl
}

function createElemKey() {

  for (let i = 0; i < keyData.length; i++) {

    const keyboardKeys = createDomElem('div', '', 'keyboard__keys');
      keyboard.appendChild(keyboardKeys);

    for (let j = 0; j < keyData[i].length; j++) {
      const currentKey = keyData[i][j];

      const key = createDomElem('div', currentKey.key.ru, 'keyboard__key', currentKey.code);
...
//   keyboard_key.forEach((e) => e.classList.toggle('active_key'))
// })
...
// const listenerKeyboard = document.querySelector('.keyboard');
// const allKey = document.querySelector('keyboard__key');

// listenerKeyboard.addEventListener('click', (event) => {
